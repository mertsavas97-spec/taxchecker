      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Copy registry and launch content into Supabase. Safe to run multiple
            times — existing rows are skipped unless you force a full refresh.
          </p>
        </div>
      ...
      {result ? (
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          ...
          {result.errors.length > 0 ? (
            <p className="text-tc-liability">
              {result.errors.length} error(s): {result.errors.join(' · ')}
            </p>
          ) : (
            <p className="text-tc-savings">Seed sync completed successfully.</p>
          )}
        </div>
      ) : null}
    </div>
